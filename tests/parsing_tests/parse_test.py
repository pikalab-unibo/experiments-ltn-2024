...
predicates = {"Man": Man(), "Women": Women(), "Person" : Person(), "Func" : Func()}
...
class TestParsing(unittest.TestCase):
    ...
    def test_function_expression(self):
        self._test_expression('Man(x)', value=torch.tensor([1.0]))
    ...
    def test_more_than_expression(self):
        self._test_expression('Man(x,z1) and (z1 > 0)', value=torch.tensor([1.0]))
    ...
    def test_complex_comparison_expression(self):
        self._test_expression('Man(x) and ( Man(x,z1) and (z1 > 0))', value=torch.tensor([1.0]))

    def test_flip_less_than_expression(self):
        self._test_expression('Man(x,z1) and z1 < 3', value=torch.tensor([1.0]))

    def test_flip_more_than_expression(self):
        self._test_expression('(z1 <= 1) and Man(x,z1)', value=torch.tensor([1.0]))

    def test_flip_less_than_or_equal_expression(self):
        self._test_expression('(z1 <= 1) and Man(x,z1)', value=torch.tensor([1.0]))

    def test_arithmetic_operations(self):
        self._test_expression('Man(x) + 3', value=torch.tensor([4.0]))
        self._test_expression('Man(x) - 1', value=torch.tensor([0.0]))
        self._test_expression('2 * Man(x)', value=torch.tensor([2.0]))
        self._test_expression('Man(x) / 2', value=torch.tensor([.5]))

    def test_nested_arithmetic_operations(self):
        self._test_expression('(Func(x) + 3) * 2', value=torch.tensor([10.0]))
        self._test_expression('(Func(x) - 1) / 2', value=torch.tensor([0.5]))
        self._test_expression('2 * (Func(x) + 1)', value=torch.tensor([6.0]))
        self._test_expression('Func(x) + 2 + 1', value=torch.tensor([5.0]))
        self._test_expression('1 + 1 + 1 + 1 + 1', value=torch.tensor([5.0]))
        self._test_expression('Func(x) + Func(x) + Func(x)', value=torch.tensor([6.0]))

    def test_arithmetic_with_comparison(self):
        self._test_expression('(Func(x) + 2) > 3', value=torch.tensor([1.0]))  # True as tensor[1.0]
        self._test_expression('3 < (Func(x) + 2)', value=torch.tensor([1.0]))  # True as tensor[1.0]
    ...
    def test_negative_numbers(self):
        ...
        self._test_expression('-(Func(x) + 2)', value=torch.tensor([-4.0]))
        self._test_expression('Func(x) + -2', value=torch.tensor([0.0]))
    ...
    def test_negative_numbers_with_comparison(self):
        ...
        self._test_expression('-Func(x) < 0', value=torch.tensor([1.0]))  # True as tensor[1.0]
        self._test_expression('Func(x) > -1', value=torch.tensor([1.0]))  # True as tensor[1.0]
